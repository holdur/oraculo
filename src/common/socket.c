...
int make_listen_bind(uint32 ip, uint16 port)
{
	...
	if( fd == -1 )
	{
		ShowError("make_listen_bind: criação de socket falhou (código %d)!\n", sErrno);
		exit(EXIT_FAILURE);
	}
	if( fd == 0 )
	{// reserved
		ShowError("make_listen_bind: Socket #0 é reservado - Reporte isto!!!\n");
		sClose(fd);
		return -1;
	}
	if( fd >= FD_SETSIZE )
	{// socket number too big
		ShowError("make_listen_bind: Novo socket #%d e maior que o emulador pode aguentar! aumente o valor de FD_SETSIZE (atualmente %d) para seu SO fixar o problema!\n", fd, FD_SETSIZE);
		sClose(fd);
		return -1;
	}
...
int make_connection(uint32 ip, uint16 port)
{
	...
	if (fd == -1) {
		ShowError("make_connection: criação de socket falhou (código %d)!\n", sErrno);
		return -1;
	}
	if( fd == 0 )
	{// reserved
		ShowError("make_connection: Socket #0 é reservado - Por favor, reporte isto!!!\n");
		sClose(fd);
		return -1;
	}
	if( fd >= FD_SETSIZE )
	{// socket number too big
		ShowError("make_connection: O Novo pacote #%d e maior que o emulador pode usar! Incremente o valor de FD_SETSIZE (atualmente %d) para fixar o problema!\n", fd, FD_SETSIZE);
		sClose(fd);
		return -1;
	}
	...
	ShowStatus("Conectando com %d.%d.%d.%d:%i\n", CONVIP(ip), port);
