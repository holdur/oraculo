...
int npc_get_new_npc_id(void)
{
	...
	{// find next id
		...
		ShowFatalError("npc_get_new_npc_id: Todos os ids sendo utilizados. Finalizando...");
		exit(1);
	}
}
...
int npc_event_dequeue(struct map_session_data* sd)
{
	...
	{	//Failed to dequeue, couldn't set a timer.
		ShowWarning("npc_event_dequeue: Timer de eventos esta cheio !\n");
		return 0;
	}
...
int npc_event_export(char* lname, void* data, va_list ap)
{
	...
	if ((lname[0]=='O' || lname[0]=='o')&&(lname[1]=='N' || lname[1]=='n')) {
		...
		if (ev==NULL) {
			...
			ShowFatalError("npc_event_export: Erro com nome de label !\n");
			exit(EXIT_FAILURE);
		}else{
			ev->nd = nd;
			ev->pos = pos;
...
int npc_timerevent(int tid, unsigned int tick, int id, intptr data)
{
	...
	{
		ShowError("npc_timerevent: Jogador ligado(attached) nao encontrado.\n");
...
int npc_timerevent_start(struct npc_data* nd, int rid)
{
	...
	{ // Failed to attach timer to this player.
		ShowError("npc_timerevent_start: Jogador ligado nao encontrado!\n");
		return 1;
	}
...
int npc_timerevent_stop(struct npc_data* nd)
{
	...
	{
		ShowError("npc_timerevent_stop: Jogador ligado nao encontrado!\n");
		return 1;
	}
...
void npc_timerevent_quit(struct map_session_data* sd)
{
	...
	if( nd && nd->bl.type == BL_NPC )
	{
		...
		if( ev && ev->nd != nd )
		{
			ShowWarning("npc_timerevent_quit: Impossivel executar \"OnTimerQuit\", dois npcs com o mesmo nome de evento [%s]!\n",buf);
			ev = NULL;
		}
		if( ev )
		{
...
int npc_event_sub(struct map_session_data* sd, struct event_data* ev, const char* eventname)
{
	if ( sd->npc_id != 0 )
	{
		...
		ShowWarning("npc_event: lista de eventos do jogador cheia, impossivel adicionar evento '%s' !\n", eventname);
		return 1;
	}
...
int npc_scriptcont(struct map_session_data* sd, int id)
{
	...
	if(id != fake_nd->bl.id) { // Not item script
		if ((npc_checknear(sd,map_id2bl(id))) == NULL) {
			ShowWarning("npc_scriptcont: O Teste de npc_checknear falhou.\n");
			return 1;
		}
	}
...
int npc_cashshop_buy(struct map_session_data *sd, int nameid, int amount, int points)
{
	...
	{
		ShowWarning("npc_cashshop_buy: Item '%s' (%d) Tentativa de overflow no preÁo!\n", item->name, nameid);
...
/// Para o jogador comprar item nas lojas de NPC. 
///
...
static const char* npc_parse_mapflag(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	if (!strcmpi(w3, "nosave")) {
		...
		ShowError("npc_parse_mapflag: mapflag '%s' desconhecido (arquivo '%s', linha '%d').\n", w3, filepath, strline(buffer,start-buffer));
...
int npc_reload(void)
{
	...
	{// dynamic check by [random]
		...
		if (map[m].npc_num > 0)
			ShowWarning("npc_reload: %d npcs nao foram removidos no mapa %s!\n", map[m].npc_num, map[m].name);
	}
	...
	{
		ShowStatus("Carregando arquivo: %s"CL_CLL"\r", nsl->name);
		npc_parsesrcfile(nsl->name);
	}

	ShowInfo ("Leitura Completa de '"CL_WHITE"%d"CL_RESET"' NPCs:"CL_CLL"\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' Warps\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' Lojas\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' Scripts\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' Pontos de Spawn\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' Mobs em Cach\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' Mobs que n„o est„o em Cach\n",
	...
	if(!CheckForCharServer()){
		ShowStatus("Evento '"CL_WHITE"OnCharIfInit"CL_RESET"' executado em '"CL_WHITE"%d"CL_RESET"' NPCs.\n", npc_event_doall("OnCharIfInit"));
...
int do_init_npc(void)
{
	...
	{
		ShowStatus("Carregando arquivo: %s"CL_CLL"\r", file->name);
	...
	ShowInfo ("Leitura completa de '"CL_WHITE"%d"CL_RESET"' NPCs:"CL_CLL"\n"
		...
		"\t-'"CL_WHITE"%d"CL_RESET"' Mobs fora de cahce\n",
