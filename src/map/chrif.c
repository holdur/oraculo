...
int chrif_setip(const char* ip)
{
	...
	if (!char_ip) {
		...
		ShowWarning("Falha ao resolver o endereco do servidor de personagens (%s)\n", ip);
		return 0;
	}
...
int chrif_save(struct map_session_data *sd, int flag)
{
	...
	if (flag && sd->state.active) //Store player data which is quitting.
	{
		...
		if (!chrif_auth_logout(sd, flag==1?ST_LOGOUT:ST_MAPCHANGE))
			ShowError("chrif_save: Falha ao configurar jogador %d:%d para logout!\n", sd->status.account_id, sd->status.char_id);
	}
...
int chrif_changemapserverack(int account_id, int login_id1, int login_id2, int char_id, short map_index, short x, short y, uint32 ip, uint16 port)
{
	...
	if (!login_id1) {
		...
		ShowError("falha na mudanca de servidor de mapas.\n");
...
int chrif_connectack(int fd)
{
	...
	if (RFIFOB(fd,2)) {
		ShowFatalError("Falha ao conectar com o servidor de personagens %d.\n", RFIFOB(fd,2));
		exit(EXIT_FAILURE);
	}

	ShowStatus("Conexao com o servidor de personagens foi feita com sucesso (Conexao: '"CL_WHITE"%d"CL_RESET"').\n",fd);
	chrif_state = 1;
	chrif_connected = 1;
...
{
	if (RFIFOB(fd,2)) {
		ShowFatalError("chrif : Falha ao enviar lista de mapas ao servidor de personagens %d\n", RFIFOB(fd,2));
		exit(EXIT_FAILURE);
	}

	memcpy(wisp_server_name, RFIFOP(fd,3), NAME_LENGTH);
	ShowStatus("Envio de mapas concluido. O servidor de mapas esta aberto.\n");
	chrif_state = 2;
...
void chrif_authok(int fd)
{
	...
	{
		ShowError("chrif_authok: Tamanho de dados incompativel! %d != %d\n", RFIFOW(fd,2) - 24, sizeof(struct mmo_charstatus));
		return;
	}
...
int auth_db_cleanup_sub(DBKey key,void *data,va_list ap)
{
	...
	if(DIFF_TICK(gettick(),node->node_created)>60000) {
		...
		default:
			...
			ShowInfo("auth_db: Nodo (estado %s) tempo expirou para %d:%d\n", states[node->state], node->account_id, node->char_id);
...
int chrif_changesex(struct map_session_data *sd)
{
	...
	clif_displaymessage(sd->fd, "… necess·rio desconecta-se para realizar o pedido de mudanÁa de sexo...");
...
static void chrif_char_ask_name_answer(int acc, const char* player_name, uint16 type, uint16 answer)
{
	...
	case 5 : action = "mudar o sexo de"; break;
	default: action = "???"; break;
	}
	
	switch( answer ) {
	//case 0 : sprintf(output, "Login-server has been asked to %s the player '%.*s'.", action, NAME_LENGTH, player_name); break;
	case 0 : sprintf(output, "O servidor de login foi solicitado para %s o jogador '%.*s'.", action, NAME_LENGTH, player_name); break;
	...
	case 2 : sprintf(output, "Seu nivel de GM nao te autoriza a %s o jogador '%.*s'.", action, NAME_LENGTH, player_name); break;
	//case 3 : sprintf(output, "Login-server is offline. Impossible to %s the player '%.*s'.", action, NAME_LENGTH, player_name); break;
	case 3 : sprintf(output, "Servidor de login desligado. Impossivel %s o jogador '%.*s'.", action, NAME_LENGTH, player_name); break;
...
int chrif_changedsex(int fd)
{
	...
	if (sd) { //Normally there should not be a char logged on right now!
		...
		clif_displaymessage(sd->fd, "Seu sexo foi mudado com sucesso (necessario desconectar do servidor)...");
...
int chrif_accountban(int fd)
{
	...
	{ 
		...
		case 1: clif_displaymessage(sd->fd, "Sua conta est· 'n„o registrada'."); break;
		//case 2: clif_displaymessage(sd->fd, "Your account has an 'Incorrect Password'..."); break;
		case 2: clif_displaymessage(sd->fd, "Sua conta est· 'com uma senha incorreta'..."); break;
		...
		case 6: clif_displaymessage(sd->fd, "Seu hexed n„o est· atualizado."); break;
		//case 7: clif_displaymessage(sd->fd, "Your account has been prohibited to log in."); break;
		case 7: clif_displaymessage(sd->fd, "Sua conta foi proibida de logar."); break;
		//case 8: clif_displaymessage(sd->fd, "Server is jammed due to over populated."); break;
		case 8: clif_displaymessage(sd->fd, "O servidor est· paralizado por estar cheio."); break;
		//case 9: clif_displaymessage(sd->fd, "Your account has not more authorised."); break;
		case 9: clif_displaymessage(sd->fd, "Sua conta n„o tem mais autorizaÁ„o."); break;
		//case 100: clif_displaymessage(sd->fd, "Your account has been totally erased."); break;
		case 100: clif_displaymessage(sd->fd, "Sua conta foi apagada."); break;
		//default:  clif_displaymessage(sd->fd, "Your account has not more authorised."); break;
		default:  clif_displaymessage(sd->fd, "Sua conta n„o tem mais autorizaÁ„o."); break;
		}
	}
