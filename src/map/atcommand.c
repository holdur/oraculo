...
int atcommand_storage(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (storage_storageopen(sd,1) == 1)	//Edição Cronus
	{	//Already open.
...
int atcommand_guildstorage(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	storage_guild_storageopen(sd,1);
...
int atcommand_jobchange(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d", &job, &upper) < 1)
	{
		...
		if (!found) {
			...
			clif_displaymessage(fd, "   1 Espadachim         8 Sacerdote         15 Monge             22 N/A");
			...
			clif_displaymessage(fd, "   5 Mercador          12 Mercenário        19 Bardo             26 N/A");
			...
			clif_displaymessage(fd, "4027 Bebê Noviço     4034 Bebê Caçador      4041 Bebê Alquimista 4048 N/A");
			...
			clif_displaymessage(fd, "[id]: -1 (padrão) para determinar automaticamente o 'nível', 0 para profissão normal, 1 para profissão superior.");
			return -1;
		}
	}
...
int atcommand_go(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%11s", map_name) < 1 || town < 0 || town >= ARRAYLENGTH(data)) {
		...
		clif_displaymessage(fd, " 24=Veins                25=Moscóvia    26=Midgard Camp");
...
int atcommand_mapinfo(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (map[m_id].flag.town)
		clif_displaymessage(fd, "Cidade");

	if (battle_config.autotrade_mapflag == map[m_id].flag.autotrade)
		clif_displaymessage(fd, "Autotrade Permitido");
	else
		clif_displaymessage(fd, "Autotrade Desativado");
...
int atcommand_rates(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	snprintf(buf, CHAT_SIZE_MAX, "Taxa da experiência: Base %.2fx / Job %.2fx",
		battle_config.base_exp_rate/100., battle_config.job_exp_rate/100.);
	clif_displaymessage(fd, buf);
	snprintf(buf, CHAT_SIZE_MAX, "Taxa de Drop Normal: Comuns %.2fx / Cura %.2fx / Usáveis %.2fx / Equipamento %.2fx / Carta %.2fx",
	...
	clif_displaymessage(fd, buf);
	snprintf(buf, CHAT_SIZE_MAX, "Taxa de Drop de Chefes: Comuns %.2fx / Cura %.2fx / Usáveis %.2fx / Equipamento %.2fx / Carta %.2fx",
	...
	clif_displaymessage(fd, buf);
	snprintf(buf, CHAT_SIZE_MAX, "Taxa de Outros Drops: MvP %.2fx / Baseado em Carta %.2fx / Baús do Tesouro %.2fx",
