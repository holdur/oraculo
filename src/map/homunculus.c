...
int merc_hom_evolution(struct homun_data *hd)
{
	...
	if (!merc_hom_change_class(hd, hd->homunculusDB->evo_class)) {
		ShowError("merc_hom_evolution: Nao e possivel evoluir o homunc de %d para %d", hd->homunculus.class_, hd->homunculusDB->evo_class);
		return 0;
	}
...
int merc_hom_alloc(struct map_session_data *sd, struct s_homunculus *hom)
{
	...
	if(i < 0) {
		ShowError("merc_hom_alloc: classe desconhecida [%d] para homunculus '%s', solicitando deletacao.\n", hom->class_, hom->name);
		sd->status.hom_id = 0;
...
int read_homunculusdb(void)
{
	...
	for(i = 0; i<2; i++)
	{
		...
		if(!fp){
			if(i != 0)
				continue;
			ShowError("read_homunculusdb : nao e possivel ler %s\n", line);
			return -1;
		}
		...
		while(fgets(line, sizeof(line), fp) && j < MAX_HOMUNCULUS_CLASS)
		{
			...
			if (k < 50 )
			{
				ShowError("read_homunculusdb : Numero de colunas incorretas em %s, homunculus %d. %d colunas lidas, 50 sao necessarias.\n", filename[i], j+1, k);
				continue;
			}
			...
			{
				ShowError("read_homunculusdb : Classe invalida %d (%s)\n", classid, filename[i]);
				continue;
			}
			db = &homunculus_db[j];
		...
		if (j > MAX_HOMUNCULUS_CLASS)
			ShowWarning("read_homunculusdb: Alcancado o numero maximo de homunculus [%d]. Homunculos restantes nao foram lidos.\n ", MAX_HOMUNCULUS_CLASS);
		fclose(fp);
		ShowStatus("Leitura de '"CL_WHITE"%d"CL_RESET"' homunculus em '"CL_WHITE"db/%s"CL_RESET"' concluida.\n",j,filename[i]);
	}
	return 0;
}
...
int read_homunculus_skilldb(void)
{
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if (j == MAX_SKILL_TREE)
		{
			ShowWarning("Nao foi possivel carregar a habilidade %d na arvore de habilidades do homunculus %d. Numero maximo de habilidades por classe foi alcancado.\n", k, classid);
			continue;
		}
...
void read_homunculus_expdb(void)
{
	...
	for(i=0; i<2; i++){
		...
		{
			ShowWarning("read_hexptbl: Alcancado o nivel maximo no exp_homun [%d]. As linhas restantes nao foram lidas.\n ", MAX_LEVEL);
			hexptbl[MAX_LEVEL - 1] = 0;
		}
		fclose(fp);
		ShowStatus("Leitura de '"CL_WHITE"%d"CL_RESET"' niveis em '"CL_WHITE"%s"CL_RESET"' concluida.\n", j, filename[i]);
	}
}
