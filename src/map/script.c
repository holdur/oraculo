...
BUILDIN_FUNC(menu)
{
	...
	if( sd->state.menu_or_input == 0 )
	{
		...
		clif_scriptmenu(sd, st->oid, StringBuf_Value(&buf)); //Envia o menu ao client
		StringBuf_Destroy(&buf);
...
BUILDIN_FUNC(cutin)
{
	...
	st->cutin = ( (script_getnum(st,3) != 255) ? 1 : 0); // [Keoy]
...
BUILDIN_FUNC(getitem2)
{
	...
	*/
	...
	{
		expire_time = script_getnum(st,12);     

		// Se o expire_time possuir algum valor "absurdo"
		if( expire_time < 0 || expire_time >= INT_MAX )
			expire_time = 0;
	}
	...
	if(nameid > 0) {
		...
		if( expire_time )
			item_tmp.expire_time = (unsigned int)(time(NULL) + expire_time);
		...
		for (i = 0; i < amount; i += get_count)
		{
			...
			if (!pet_create_egg(sd, nameid))
			{
				...
				if( expire_time )
				{
					clif_rental_time(sd->fd, nameid, expire_time);
					pc_inventory_rental_add(sd, expire_time);
				}
			}
		}
...
BUILDIN_FUNC(guildopenstorage)
{
	...
	ret = storage_guild_storageopen(sd,1);
	script_pushint(st,ret);
	return 0;
}
...
BUILDIN_FUNC(getinventorylist)
{
	...
	for(i=0;i<MAX_INVENTORY;i++){
		if(sd->status.inventory[i].nameid > 0 && sd->status.inventory[i].amount > 0){
			...
			pc_setreg(sd,add_str("@inventorylist_expire")+(j<<24),sd->status.inventory[i].expire_time); // Retorna o tempo que falta para o item sumir (rentitem) [TecnoCronus]
			j++;
		}
	}
