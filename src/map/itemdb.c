...
int itemdb_searchrandomid(int group)
{
	if(group<1 || group>=MAX_ITEMGROUP) {
		ShowError("itemdb_searchrandomid: Grupo invalido de ID %d\n", group);
		return UNKNOWN_ITEM_ID;
	}
	if (itemgroup_db[group].qty)
		return itemgroup_db[group].nameid[rand()%itemgroup_db[group].qty];
	
	ShowError("itemdb_searchrandomid: Sem entradas para o Grupo de ID %d\n", group);
	return UNKNOWN_ITEM_ID;
}
...
struct item_data* itemdb_search(int nameid)
{
	...
	if( id == NULL )
	{
		ShowWarning("itemdb_search: Item de ID %d nao existe no item_db.\n", nameid);
		id = &dummy_item;
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if (j<3) {
			if (j>1) //Or else it barks on blank lines...
				ShowWarning("itemdb_read_itemgroup: Campos insuficientes para a entrada em %s:%d\n", filename, ln);
			continue;
		}
		groupid = atoi(str[0]);
		if (groupid < 0 || groupid >= MAX_ITEMGROUP) {
			ShowWarning("itemdb_read_itemgroup: Grupo %d invalido em %s:%d\n", groupid, filename, ln);
			continue;
		}
		nameid = atoi(str[1]);
		if (!itemdb_exists(nameid)) {
			ShowWarning("itemdb_read_itemgroup: Item %d nao-existente em %s:%d\n", nameid, filename, ln);
			continue;
		}
		k = atoi(str[2]);
		if (itemgroup_db[groupid].qty+k >= MAX_RANDITEM) {
			ShowWarning("itemdb_read_itemgroup: Grupo %d esta cheio (%d entradas) em %s:%d\n", groupid, MAX_RANDITEM, filename, ln);
			continue;
		}
		for(j=0;j<k;j++)
...
static bool itemdb_parse_dbrow(char** str, const char* source, int line, int scriptopt)
{
	...
	*/
	...
	if( nameid <= 0 )
	{
		ShowWarning("itemdb_parse_dbrow: ID %d invalido na linha %d de \"%s\", ignorando.\n", nameid, line, source);
		return false;
	}
	...
	if ( !str[4][0] && !str[5][0])
	{  
		ShowWarning("itemdb_parse_dbrow: Sem preco de compra/vebda definido para o item %d (%s), usando 20/10z\n",       nameid, id->jname);
		id->value_buy = 20;
		id->value_sell = 10;
	} else
	*/
	if (id->value_buy/124. < id->value_sell/75.)
		ShowWarning("itemdb_parse_dbrow: Preco de compra/venda [%d/%d] do item %d (%s) permite fazer zeny exploit comprando/vendendo com desconto/superfaturar!\n",
	...
	{
		ShowWarning("itemdb_parse_dbrow: Item %d (%s) especifica %d slots, mas o servidor so suporta ate %d. Usando %d slots.\n", nameid, id->jname, id->slot, MAX_SLOTS, MAX_SLOTS);
		id->slot = MAX_SLOTS;
	}
	...
	{
		ShowWarning("Item %d (%s) e um equipamento sem localizacao de onde equipar! Fazendo ele um item do tipo etc.\n", nameid, id->jname);
		id->type = IT_ETC;
	}
...
static int itemdb_readdb(void)
{
	...
	for( fi = 0; fi < ARRAYLENGTH(filename); ++fi )
	{
		...
		if( fp == NULL )
		{
			ShowWarning("itemdb_readdb: Arquivo nao encontrado \"%s\", ignorando.\n", path);
			continue;
		}
		...
		while(fgets(line, sizeof(line), fp))
		{
			...
			if( p == NULL )
			{
				ShowError("itemdb_readdb: Colunas insuficientes na linha %d do \"%s\" (item com id %d), ignorando.\n", lines, path, atoi(str[0]));
				continue;
			}
			...
			if( *p != '{' )
			{
				ShowError("itemdb_readdb: Formato invalido (coluna Script) na linha %d do \"%s\" (item com id %d), ignorando.\n", lines, path, atoi(str[0]));
				continue;
			}
			str[19] = p;
			p = strstr(p+1,"},");
			...
			if( *p != '{' )
			{
				ShowError("itemdb_readdb: Formato invalido (coluna OnEquip_Script) na linha %d do \"%s\" (item com id %d), ignorando.\n", lines, path, atoi(str[0]));
				continue;
			}
			str[20] = p;
			p = strstr(p+1,"},");
			...
			if( *p != '{' )
			{
				ShowError("itemdb_readdb: Formato invalido (coluna OnUnequip_Script) na linha %d do \"%s\" (item com id %d), ignorando.\n", lines, path, atoi(str[0]));
				continue;
			}
			str[21] = p;
