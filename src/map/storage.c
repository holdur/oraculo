 ...
 * 1 - fail
 *------------------------------------------*/
int storage_storageopen(struct map_session_data *sd, unsigned short int FromScript)
{
	...
	m = sd->bl.m; //Bloqueio de Storage (Até a 104)
	if (map[m].flag.nostorage && !FromScript) {
...
int storage_guild_storageopen(struct map_session_data* sd, unsigned short int FromScript) 
{
	...
	m = sd->bl.m;
    if (map[m].flag.noguildstorage && !FromScript) {
