...
char wisp_server_name[NAME_LENGTH] = "Servidor"; // can be modified in char-server configuration file
...
int map_freeblock_unlock (void)
{
	if ((--block_free_lock) == 0) {
		...
		ShowError("map_freeblock_unlock: contagem de blocos fechados < 0 !\n");
		block_free_lock = 0;
	}
...
int map_addblock(struct block_list* bl)
{
	...
	if( m < 0 || m >= map_num )
	{
		...
		ShowError("map_addblock: mapa com id invalido (%d), so %d estao carregados.\n", m, map_num);
		return 1;
	}
	if( x < 0 || x >= map[m].xs || y < 0 || y >= map[m].ys )
	{
		//ShowError("map_addblock: out-of-bounds coordinates (\"%s\",%d,%d), map is %dx%d\n", map[m].name, x, y, map[m].xs, map[m].ys);
		ShowError("map_addblock: coordenadas fora de alcance (\"%s\",%d,%d), o mapa e %dx%d\n", map[m].name, x, y, map[m].xs, map[m].ys);
		return 1;
	}
...
int map_foreachinrange(int (*func)(struct block_list*,va_list), struct block_list* center, int range, int type, ...)
{
	...
	if(bl_list_count>=BL_LIST_MAX)
		...
		ShowWarning("map_foreachinrange: contagem de blocos muito alta!\n");
...
int map_foreachinarea(int (*func)(struct block_list*,va_list), int m, int x0, int y0, int x1, int y1, int type, ...)
{
	...
	if(bl_list_count>=BL_LIST_MAX)
		...
		ShowWarning("map_foreachinarea: contagem de blocos muito alta!\n");
...
int map_foreachinmovearea(int (*func)(struct block_list*,va_list), struct block_list* center, int range, int dx, int dy, int type, ...)
{
	...
	if(bl_list_count>=BL_LIST_MAX)
		...
		ShowWarning("map_foreachinmovearea: contagem de blocos muito alta!\n");		
...
int map_foreachincell(int (*func)(struct block_list*,va_list), int m, int x, int y, int type, ...)
{
	...
	if(bl_list_count>=BL_LIST_MAX)
		...
		ShowWarning("map_foreachincell: contagem de blocos muito alta!\n");
...
// kRO.
	...
	if(bl_list_count>=BL_LIST_MAX)
		...
		ShowWarning("map_foreachinpath: contagem de blocos muito alta!\n");
...
int map_foreachinmap(int (*func)(struct block_list*,va_list), int m, int type,...)
{
	...
	if(bl_list_count>=BL_LIST_MAX)
		...
		ShowWarning("map_foreachinmap: contagem de blocos muito alta!\n");
...
int map_get_new_object_id(void)
{
	...
	if( i == last_object_id )
	{
		...
		ShowError("map_addobject: sem id de objetos livre!\n");
		return 0;
	}
...
int map_search_freecell(struct block_list *src, int m, short *x,short *y, int rx, int ry, int flag)
{
	...
	if( !src && (!(flag&1) || flag&2) )
	{
		...
		ShowDebug("map_search_freecell: Uso incorreto! Quando src e NULL, a flag deve ser &1 e nao &2\n");
		return 0;
	}
...
bool map_addnpc(int m,struct npc_data *nd)
{
	...
	if( map[m].npc_num == MAX_NPC_PER_MAP )
	{
		...
		ShowWarning("excesso de NPCs em um mapa %s\n",map[m].name);
		return false;
	}
...
int map_removemobs_timer(int tid, unsigned int tick, int id, intptr data)
{
	...
	{	//Incorrect map id!
		...
		ShowError("map_removemobs_timer: timer %d pontos para mapa invalido %d\n",tid, m);
		return 0;
	}
	...
	if (battle_config.etc_log && count > 0)
		...
		ShowStatus("Mapa %s: Removidos '"CL_WHITE"%d"CL_RESET"' monstros.\n",map[m].name, count);
...
inline static struct mapcell map_gat2cell(int gat)
{
	...
	default:
		...
		ShowWarning("map_gat2cell: tipo de gat desconhecido '%d'\n", gat);
		break;
	}
...
static int map_cell2gat(struct mapcell cell)
{
	...
	ShowWarning("map_cell2gat: celula nao tem um gat correspondente\n");
...
void map_setcell(int m, int x, int y, cell_t cell, bool flag)
{
	...
	switch( cell ) {
		...
		default:
			...
			ShowWarning("map_setcell: tipo invalido de celula '%d'\n", (int)cell);
			break;
	}
}
...
int map_setipport(unsigned short mapindex, uint32 ip, uint16 port)
{
	...
	if(ip == clif_getip() && port == clif_getport()) {
		...
		ShowFatalError("map_setipport : recebida informacao de que o servidor de mapas DEVE ter o mapa '%s', mas ele nao esta carregado.\n",mapindex_id2name(mapindex));
		exit(EXIT_FAILURE);
	}
	mdos->ip   = ip;
	mdos->port = port;
...
int map_addmap(char* mapname)
{
	...
	if( map_num >= MAX_MAP_PER_SERVER - 1 )
	{
		...
		ShowError("Impossivel adicionar mapa '"CL_WHITE"%s"CL_RESET"', o limite de mapas foi alcancado.\n",mapname);
		return 1;
	}
...
int map_waterheight(char* mapname)
{
	...
	ShowWarning("Falha ao encontrar nivel de agua para (%s)\n", mapname, fn);
	return NO_WATER;
}
...
int map_readallmaps (void)
{
	...
	else
	{
		...
		if( (fp = fopen(map_cache_file, "rb")) == NULL )
		{
			...
			ShowFatalError("Nao foi possivel abrir o mapcache "CL_WHITE"%s"CL_RESET"\n", map_cache_file);
		...
		if(!map_cache_buffer) {
			...
			ShowFatalError("Falhar em inicializar o mapcache (%s)..\n", map_cache_file);
			exit(EXIT_FAILURE);
		}
	}
	...
	for(i = 0; i < map_num; i++)
	{
		...
		if (uidb_get(map_db,(unsigned int)map[i].index) != NULL)
		{
			...
			ShowWarning("O mapa %s ja foi carregado!"CL_CLL"\n", map[i].name);
			if (map[i].cell) {
				aFree(map[i].cell);
				map[i].cell = NULL;	
			}
	...
	ShowInfo("Carregados '"CL_WHITE"%d"CL_RESET"' mapas com sucesso."CL_CLL"\n",map_num);
	instance_start = map_num; // Next Map Index will be instances

	if (maps_removed)
		ShowNotice("Maps removidos: '"CL_WHITE"%d"CL_RESET"'\n",maps_removed);
...
int parse_console(char* buf)
{
	...
	if( n == 5 && strcmpi("admin",type) == 0 ){
		...
		ShowNotice("Para usar comandos de GM:\n");
		//ShowInfo("admin:<gm command>:<map of \"gm\"> <x> <y>\n");
		ShowInfo("admin:<comando>:<mapa do \"gm\"> <x> <y>\n");
		//ShowInfo("You can use any GM command that doesn't require the GM.\n");
		ShowInfo("Voce pode usar qualquer comando que nao precise do GM.\n");
		//ShowInfo("No using @item or @warp however you can use @charwarp\n");
		ShowInfo("Sem utilizar o @item ou @warp mas voce pode usar o @charwarp\n");
		//ShowInfo("The <map of \"gm\"> <x> <y> is for commands that need coords of the GM\n");
		ShowInfo("O <mapa do \"gm\"> <x> <y> e para comandos que precisam das coordenadas do GM\n");
		//ShowInfo("IE: @spawn\n");
		ShowInfo("IE: @spawn\n");
		//ShowInfo("To shutdown the server:\n");
		ShowInfo("Para desativar o servidor:\n");
...
int map_config_read(char *cfgName)
{
	...
	if( fp == NULL )
	{
		...
		ShowError("Arquivo de configuracao nao encontrado em: %s\n", cfgName);
		return 1;
	}
	...
	while( fgets(line, sizeof(line), fp) )
	{
		...
		*ptr = '\0';
		...
		if(strcmpi(w1,"console_silent")==0) {
			ShowInfo("Configuracao do console (silent): %d\n", atoi(w2));
			msg_silent = atoi(w2);
		} else
		...
		if (strcmpi(w1, "console") == 0) {
			...
			if (console)
				ShowNotice("Comandos de console ativados.\n");
		} else
		...
		if (strcmpi(w1, "import") == 0)
			...
			ShowWarning("Configuracao invalida '%s' no arquivo %s\n", w1, cfgName);
	}
...
int map_sql_close(void)
{
	...
	ShowStatus("Finalizando a conexao com a database do servidor....\n");
	...
	if (log_config.sql_logs)
	{
		...
		ShowStatus("Finalizando conexao com a database de logs....\n");
...
int log_sql_init(void)
{
	...
	ShowInfo(""CL_WHITE"[SQL]"CL_RESET": Conectando a database de Logs "CL_WHITE"%s"CL_RESET" Em "CL_WHITE"%s"CL_RESET"...\n",log_db_db,log_db_ip);
	if ( SQL_ERROR == Sql_Connect(logmysql_handle, log_db_id, log_db_pw, log_db_ip, log_db_port, log_db_db) )
		exit(EXIT_FAILURE);
	//ShowStatus(""CL_WHITE"[SQL]"CL_RESET": Successfully '"CL_GREEN"connected"CL_RESET"' to Database '"CL_WHITE"%s"CL_RESET"'.\n", log_db_db);
	ShowStatus(""CL_WHITE"[SQL]"CL_RESET": Conectado com '"CL_GREEN"sucesso"CL_RESET"' a database '"CL_WHITE"%s"CL_RESET"'.\n", log_db_db);
...
void do_abort(void)
{
	...
	if (run) {
		...
		ShowFatalError("O servidor caiu enquanto tentava salvar os personagens. Informacao dos personagens nao pode ser salva!\n");
		return;
	}
	run = 1;
	if (!chrif_isconnected())
	{
		if (pc_db->size(pc_db))
			...
			ShowFatalError("O servidor caiu sem uma conexao com o servidor de personagens, %u personagens nao foram salvos!\n", pc_db->size(pc_db));
		return;
	}
	//ShowError("Server received crash signal! Attempting to save all online characters!\n");
	// Nota de traduÁ„o: N„o achei uma traduÁ„o v·lida para crash nesse caso
	ShowError("O servidor recebeu um sinal de crash! Tentando salvar todos os personagens!\n");
...
void map_versionscreen(int flag)
{
	...
	if (ATHENA_RELEASE_FLAG) ShowNotice("Esta versao nao e para ser compartilhada.\n");
	if (flag) exit(EXIT_FAILURE);
}
...
int do_init(int argc, char *argv[])
{
	...
	if (!map_ip_set || !char_ip_set) {
		...
		ShowError("Nem todos os IPs em map_athena.conf foram configurados, detectando...\n");

		if (naddr_ == 0)
			//ShowError("Unable to determine your IP address...\n");
			ShowError("Falha ao detectar endereco de IP...\n");
		else if (naddr_ > 1)
			//ShowNotice("Multiple interfaces detected...\n");
			ShowNotice("Mais de uma interface foi detectada...\n");

		ShowInfo("Padronizando %s como nosso endereco de IP\n", ip_str);
	...
	ShowStatus("O servidor esta '"CL_GREEN"pronto"CL_RESET"' e escutando a porta '"CL_WHITE"%d"CL_RESET"'.\n\n", map_port);
