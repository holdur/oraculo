...
int status_check_skilluse(struct block_list *src, struct block_list *target, int skill_num, int flag)
{
	...
	if(sc && sc->count)
	{
		...
		if (skill_num && //Do not block item-casted skills.
			...
			// Lamento de loki bloqueia todos os tipos de skill, vide bRO [Keoy]
			// http://games.levelupgames.uol.com.br/ragnarok/guia-do-jogo/personagens/clas-se-2-2/bardo/habilidades
			if(sc->data[SC_ROKISWEIL] && skill_num != BD_ADAPTATION)
				return 0;	
	...
	hide_flag = flag?OPTION_HIDE:(OPTION_HIDE|OPTION_CLOAK|OPTION_CHASEWALK);
 	...
 	//VocÍ n„o pode se esconder de skills de 'ground'
	//Nem da dissonancia, ela te tira do hide [Keoy]
...
int status_change_clear(struct block_list* bl, int type)
{
	...
	for(i = 0; i < SC_MAX; i++)
	{
		...
		case SC_DEXFOOD:
		case SC_LUKFOOD:
			if(sc->data[i]->val1 == 10) // Quando È igual a 10 o status n„o sai apÛs a morte [Keoy]
				continue;
		}
