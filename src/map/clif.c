...
void clif_setbindip(const char* ip)
{
	...
	if (bind_ip) {
		...
		ShowWarning("Falha ao resolver o endereco de ip do Servidor de Mapas! (%s)\n", ip);
	}
}
...
int clif_send_sub(struct block_list *bl, va_list ap)
{
	...
	if (WFIFOP(fd,0) == buf) {
		...
		ShowError("AVISO: Uso invalido da funcao clif_send\n");
		ShowError("       Pacote x%4x usa um WFIFO de um player ao inves de usar um buffer.\n", WBUFW(buf,0));
...
int clif_hpmeter_sub(struct block_list *bl, va_list ap)
{
	...
	// N„o podemos mandar o packet para o prÛprio player [Keoy]
	if( !tsd->fd || tsd == sd )
		return 0;
...
void clif_parse_WisMessage(int fd, struct map_session_data* sd)
{
	...
	if( dstsd->state.autotrade == 1 )
	{
		...
		sprintf(output, "%s est· em autotrade e n„o pode receber mensagens.", dstsd->status.name);
...
void clif_parse_NpcClicked(int fd,struct map_session_data *sd)
{
	...
	switch (bl->type) {
		...
		case BL_PC:
			...
			break;
		case BL_NPC:
			if( bl->m != -1 )// O player n„o pode clicar em npcs em floating (flutuantes -1) tentativa de hack
				npc_click(sd,(TBL_NPC*)bl);
			else
			{ //Tentativa de hack, kickar o jogador [Keoy]
				ShowWarning("Tentativa de hack, clicou em um npc flutuante (AID:%d, CID:%d, nome:'%s')!\n", sd->bl.id, sd->status.char_id, sd->status.name);
...
// Mostra experiÍncia ganha
// tipo = 1 -> base_exp
// tipo = 2 -> job_exp
// flag = 0 -> exp normal ganha/perdida
// flag = 1 -> exp de quest ganha/perdida
	...
	if (session[fd]->flag.eof) {
		if (sd) {
			if (sd->state.autotrade) {
				...
				sd->fd = 0;
				ShowInfo("%sPersonagem '"CL_WHITE"%s"CL_RESET"' deslogou (usando @autotrade).\n", (pc_isGM(sd))?"GM ":"", sd->status.name);
			} else
			if (sd->state.active) {
				 // Player logout display [Valaris]
				ShowInfo("%sPersonagem '"CL_WHITE"%s"CL_RESET"' deslogou.\n", (pc_isGM(sd))?"GM ":"", sd->status.name);
				clif_quitsave(fd, sd);
			} else {
			...
			ShowInfo("Fechada conexao de '"CL_WHITE"%s"CL_RESET"'.\n", ip2str(session[fd]->client_addr, NULL));
		}
		do_close(fd);
		return 0;
	}
