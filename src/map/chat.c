...
int chat_createpcchat(struct map_session_data* sd, const char* title, const char* pass, int limit, bool pub)
{
	...
	if((battle_config.no_prt_chating) && (sd->bl.m == map_mapname2mapid("prontera.gat")
	...
	{ // Não abrir Chats no meio da Rua [Minos e RoM]
...
int chat_leavechat(struct map_session_data* sd, bool kicked)
{
	...
	{	// Set and announce new owner
		...
		cd->bl.m=cd->usersd[0]->bl.m;

		if((battle_config.no_prt_chating) && (cd->bl.m == map_mapname2mapid("prontera.gat")
			&& ((cd->bl.x >= 148 && cd->bl.x <= 163 && cd->bl.y >= 130 && cd->bl.y <= 267)
			|| (cd->bl.x >= 82 && cd->bl.x <= 229 && cd->bl.y >= 200 && cd->bl.y <= 207)
			|| ((cd->bl.y <= 200 && cd->bl.x <= 155) && (cd->bl.x >= ((cd->bl.y-168)*-1)+155))
			|| ((cd->bl.y <= 200 && cd->bl.x >= 156) && (cd->bl.x <= (cd->bl.y-168)+156))
			|| ((cd->bl.y >= 207 && cd->bl.x <= 155) && (cd->bl.x >= 155-(239-cd->bl.y)))
			|| ((cd->bl.y >= 207 && cd->bl.x >= 156) && (cd->bl.x <= 156+(239-cd->bl.y))))
		))
			return 0; // Não mostrar chat nas ruas de prontera [Wilk Maia]
