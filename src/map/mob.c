...
int mob_get_random_id(int type, int flag, int lv)
{
	...
	if(type < 0 || type >= MAX_RANDOMMONSTER) {
		ShowError("mob_get_random_id: Tipo invalido de monstro aleartorio. (%d)\n", type);
		return 0;
	}
	do {
		if (type)
...
bool mob_ksprotected (struct block_list *src, struct block_list *target)
{
	...
	do {
		...
		if( DIFF_TICK(pl_sd->ks_floodprotect_tick, tick) <= 0 )
		{
			...
			sprintf(output, "[Cuidado! %s est· tentando fazer um KS em vocÍ!]", sd->status.name);
...
int mob_spawn_bg(const char* mapname, short x, short y, const char* mobname, int class_, const char* event, int bg_id)
{
	...
	if( (x <= 0 || y <= 0) && !map_search_freecell(NULL, m, &x, &y, -1,-1, 1) )
	{
		...
		ShowWarning("mob_spawn_bg: Nao foi possivel encontrar uma celula de invocacao para o guardiao %d (bg_id %d) no mapa %s\n",class_, bg_id, map[m].name);
		return 0;
	}
...
int mob_randomwalk(struct mob_data *md,unsigned int tick)
{
	...
	if(i==retrycount){
		...
		if(md->move_fail_count>1000){
			...
			ShowWarning("Monstro nao pode se mover. Invocacao aleatoria %d, classe = %d, em %s (%d,%d)\n",md->bl.id,md->class_,map[md->bl.m].name, md->bl.x, md->bl.y);
			md->move_fail_count=0;
			mob_spawn(md);
		}
		return 0;
...
int mob_guardian_guildchange(struct block_list *bl,va_list ap)
{
	...
	{	//Properly remove guardian info from Castle data.
		...
		ShowError("mob_guardian_guildchange: Novo cla nao existe! (id %d)\n", md->guardian_data->guild_id);
...
static bool mob_parse_dbrow(char** str)
{
	...
	if (class_ <= 1000 || class_ > MAX_MOB_DB) {
		...
		ShowWarning("Monstro com o ID: %d nao carregado. O ID deve estar entre [%d-%d]\n", class_, 1000, MAX_MOB_DB);
		return false;
	}
	if (pcdb_checkid(class_)) {
		//ShowWarning("Mob with ID: %d not loaded. That ID is reserved for player classes.\n");]
		ShowWarning("Monstro com o ID: %d nao carregado. ID reservado para classes de jogadores.\n");
		return false;
	}
	...
	if (class_ >= MOB_CLONE_START && class_ < MOB_CLONE_END) {
		...
		ShowWarning("Monstro com o ID: %d nao carregado. Os valores %d-%d sao reservados para clones. Por favor aumente a MAX_MOB_DB (%d)\n", class_, MOB_CLONE_START, MOB_CLONE_END-1, MAX_MOB_DB);
		return false;
	}
	...
	if (status->def_ele >= ELE_MAX) {
		...
		ShowWarning("Monstro com o ID: %d tem um tipo invalido de elemento %d (maximo de elementos e %d)\n", class_, status->def_ele, ELE_MAX-1);
		status->def_ele = ELE_NEUTRAL;
	}
	if (status->ele_lv < 1 || status->ele_lv > 4) {
		//ShowWarning("Mob with ID: %d has invalid element level %d (max is 4)\n", class_, status->ele_lv);
		ShowWarning("Monstro com o ID: %d tem um nivel invalido de elemento %d (maximo e 4)\n", class_, status->ele_lv);
		status->ele_lv = 1;
	}
	...
	for( fi = 0; fi < ARRAYLENGTH(filename); ++fi )
	{
		...
		while(fgets(line, sizeof(line), fp))
		{
			...
			if(i < 38 + 2*MAX_MOB_DROP) {
				...
				ShowWarning("mob_readdb: Colunas insuficientes para o monstro (id: %d), pulando...\n", atoi(str[0]));
				continue;
			}
		...
		while(fgets(line, sizeof(line), fp))
		{
			...
			if (i) {
				if( summon[i].qty < ARRAYLENGTH(summon[i].class_) ) //MvPs
					...
					ShowDebug("Impossivel guardar mais monstros randomicos de %s, aumente o tamanho de mob.c:summon!\n", mobfile[i]);
					break;
				}
			}
		}
...
static bool mob_parse_row_chatdb(char** str, const char* source, int line, int* last_msg_id)
{
	...
	if (msg_id <= 0 || msg_id > MAX_MOB_CHAT)
	{
		if (msg_id != *last_msg_id) {
			...
			ShowError("mob_chat: ID de chat invalido: %d em %s, linha %d\n", msg_id, source, line);
			*last_msg_id = msg_id;
		}
		return false;
	}
	...
	if(strlen(str[2])>(CHAT_SIZE_MAX-1)){
		if (msg_id != *last_msg_id) {
			...
			ShowError("mob_chat: readdb: Mensagem muito longa! Linha %d, id: %d, maximo: %d\n", line, msg_id, (CHAT_SIZE_MAX-1));
			*last_msg_id = msg_id;
		}
		return false;
	}
...
static void mob_readchatdb(void)
{
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if( j < 2 || str[2]==NULL)
		{
			...
			ShowError("mob_readchatdb: Numero de campos insuficientes para habilidade em %s, linha %d\n", arc, lines);
			continue;
		}
...
static bool mob_parse_row_mobskilldb(char** str, const char* source, int line, int* last_mob_id)
{
	...
	if (mob_id > 0 && mob_db(mob_id) == mob_dummy)
	{
		if (mob_id != *last_mob_id) {
			...
			ShowError("mob_skill: Id de monstro inexistente %d em %s, linha %d\n", mob_id, source, line);
			*last_mob_id = mob_id;
		}
		return false;
	}
	...
	{	//Prepare global skill. [Skotlex]
		...
		if( i == MAX_MOBSKILL )
		{
			if (mob_id != *last_mob_id) {
				...
				ShowError("mob_skill: readdb: muitas habilidades! Linha %d em %d[%s]\n", line, mob_id, mob_db_data[mob_id]->sprite);
				*last_mob_id = mob_id;
			}
			return false;
		}
	...
	if( j < ARRAYLENGTH(state) )
		...
		ShowWarning("mob_skill: Estado desconhecido %s em %s, linha %d\n", str[2], source, line);
		ms->state = MSS_ANY;
	}
	...
	if (j<=0 || j>MAX_SKILL_DB) //fixed Lupus
	{
		if (mob_id < 0)
			...
			ShowError("ID de habilidade invalido para todos os monstros. (%d)\n", j);
		else
			ShowError("ID de habilidade invalido(%d) para o monstro %d (%s)\n", j, mob_id, mob_db_data[mob_id]->sprite);
		return false;
	}
	ms->skill_id=j;
	...
	if( j < ARRAYLENGTH(target) )
		...
		ShowWarning("mob_skill: Alvo desconhecido %s em %s, linha %d\n", str[9], source, line);
